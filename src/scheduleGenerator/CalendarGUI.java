...
public class CalendarGUI extends javax.swing.JFrame {
	 ...
	 * This switch is used to handle each weekday.
	 * We can simplify this method by using the built in Calendar format 
	 * to get the appropriate month name.
	 * 
	 * 
	 * SWAP 2, TEAM 8
	 * // REFACTORING FOR ENHANCEMENT FROM BAD SMELL
	 * Replaced this switch statement with the built in Calendar format to get
	 * the appropriate month name (No Fowler appropriate refactoring). I could potentially use a different
	 * calendar with more than the regular months that are listed in the switch statement. This refactoring was simple
...
//		}
	...
	/* SWAP 1, TEAM 7  
	 * Smell: Long Method
	 * This method is rather long and has several different responsibilities.
	 * We should separate out some of this behavior into smaller methods 
	 ...
	 * REFACTORING FOR ENHANCEMENT FROM BAD SMELL
	 * Extract Method
	 * We moved the responsibilities that pertain to the creation of the data table and the
	 * the setting of the class's fields into their own method. This as stated above helps
	 * to clarify the methods and from there the class itself. We pulled out the parts that create
	 * a key for the table and the columns into their own methods. These will make the general code more readable.
	 * To sum up we pulled the part of the code that creates a TableModel out of fill in table
	 * and then from there pulled out as many modularized parts from that method as we could to maintain
	 * readability and make for easier debugging in the future.
	 * This would also with a little more work make changing the table formatting much easier because the code pertaining
	 * to such a change would be obviously separated into its meaningful sections
	 */
	
	//SWAP 2 Team 8 added this method
	private String[] makeColData(String tempKey){
		int i = 0;
	...
	//SWAP 2 Team 8 added this method
	public DefaultTableModel makeTableModel(int month, int year){
		...
		while (month == this.cal.get(Calendar.MONTH) + 1) {
			...
			if (this.scheduleMap.containsKey(tempKey)) {
				...
				String[] colData = makeColData(tempKey);
	...
	public void fillTableForMonth(int month, int year) {
		...
		HTMLGenerator.addMonth(this.monthName, makeTableModel(month, year));
	}
	 ...
	 * 	This behavior could be moved to Day to avoid common switch statements in multiple locations. 
	 *  Putting these labels in one location would make it much simpler to add support for multiple
	 *  languages.
	 *  
	 *  SWAP 2, TEAM 8
	 * // REFACTORING FOR ENHANCEMENT FROM BAD SMELL
	 * Instead of moving this to Day i will simplify the switch statement to something similar as the month version of this.
	 * A feature that could be added is supporting multiple languages by use of a different kind of calendar
	 * This refactoring was simple so it was fairly successful.
	 *  
	 */
