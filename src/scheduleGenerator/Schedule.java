...
public class Schedule extends Thread implements Serializable {
	...
	private ArrayList<Worker> findWorkersForJob(String job,String dayName, ArrayList<String> workersWorking)
	{
		...
		for (Worker worker : this.workerIndices.get(this.numForName(dayName))) {
			if (worker.isWorkerAvailable(job, dayName, workersWorking)) {
	...
	private void assignWorkersToJobs(Day day, ArrayList<String> jobsInOrder){
		...
		//Feature Elaboration
		/* SWAP 2 Team 8
		 * Another way you could implement this feature is to also add in worker's personal vacations. This would mean 
		 * again changing the logic of the system to accommodate WorkerSetup and Worker knowing when their vacations happen.
		 * Considering the changes that have already been made it shouldn't be difficult fix. The continued refactoring of Schedule and Worker would be beneficial
		 * to such an endeavor. So just add what I did in the comment below to this please.
		 *
		 */
		
		//Feature More Elaboration
		/*	SWAP 2 Team 8
		 * Another feature, that would make sense to have certain days of the week off ie saturday or sundat. So, for example each
		 * This was made easier by our refactoring of this class. 
		 * We were able to make some small changes to worker that would 
		 * add this feature. Without the previous refactoring changes in multiple places within schedule would have had to been made.
		 * The actual code changes were made in the worker class.The hard part about this feature would be delving into the WorkerSetup class.
		 * The next step would be to make this feature actually appear in the GUI in a meaningful way.
		 */
